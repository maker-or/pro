...
function SidebarMenuButton({
  ...
  // When both tooltip and render are provided (e.g. render={<Link href="..." />}),
  // we want the TooltipTrigger to render AS that element so it gets both
  // tooltip event handlers AND navigation behaviour. We do this by passing
  // render={userRender} to TooltipTrigger, which Base UI supports via its
  // own render-prop API. Without this, the tooltip case simply discards the
  // user-supplied render prop and the link never navigates.
  const resolvedRender = !tooltip ? render : render ? <TooltipTrigger render={render} /> : TooltipTrigger;
