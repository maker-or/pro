...
export default function ApiKeysPage() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setCreatedKey({ publicId: result.publicId, fullKey: result.fullKey, name: keyName.trim() });
      setShowForm(false);
      setKeyName('');
    } catch (err: unknown) {
          ...
          <main className="flex-1 p-6 max-w-4xl">
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Manage secret keys used to authenticate requests to your application. Keep your keys secure — never
              ...
              <Card className="mb-6 border-border">
                <CardContent className="pt-5 pb-5">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    Give your key a descriptive name so you can identify it later (e.g.&nbsp;
                    <span className="font-mono">Production App</span> or <span className="font-mono">CI Pipeline</span>
                    ).
                  </p>
                  <label className="text-xs font-medium text-foreground block mb-1.5" htmlFor="key-name">
                    Key name
                  </label>
                  <Input
                    id="key-name"
                  ...
                  <div className="flex items-start gap-2 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                        Copy and store <span className="font-medium">{createdKey.name}</span> somewhere safe. Once you
                        close this notice you cannot retrieve the full key.
                      </p>
                    </div>
                  </div>
                  ...
                  <TableBody>
                    {apiKeys.map((apiKey) => (
                      <TableRow key={apiKey._id}>
                        <TableCell className="font-medium text-xs">{apiKey.name}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            onClick={() => handleDelete(apiKey._id as Id<'secretkey'>)}
                            className="cursor-pointer"
                            title="Revoke key"
                          >
